...
public class ApiRDCNodi implements IApi<ClsRDCNodo> {
    ...
    @Override
    public boolean Post(ClsTurista user, ClsRDCNodo nodo){
        HttpRequest request = HttpRequest.newBuilder()
                .header("Authorization", FakeTokens.getToken(user))
                .header("Content-Type", "application/json")
    ...
    @Override
    public boolean Put(ClsTurista user, ClsRDCNodo nodo){
        HttpRequest request = HttpRequest.newBuilder()
                .header("Authorization", FakeTokens.getToken(user))
                .header("Content-Type", "application/json")
                .PUT(createBody(nodo))
