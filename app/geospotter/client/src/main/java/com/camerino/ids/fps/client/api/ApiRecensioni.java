...
public class ApiRecensioni implements IApi<ClsRecensione> {
    ...
    @Override
    public boolean Post(ClsTurista user, ClsRecensione nodo) {
        HttpRequest request = HttpRequest.newBuilder()
                .header("Authorization", FakeTokens.getToken(user))
                .header("Content-Type", "application/json")
    ...
    @Override
    public boolean Put(ClsTurista user, ClsRecensione nodo) {
        HttpRequest request = HttpRequest.newBuilder()
                .header("Authorization", FakeTokens.getToken(user))
                .header("Content-Type", "application/json")
                .PUT(createBody(nodo))
